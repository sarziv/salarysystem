                            ...
                            <div class="form-group">
                                <table class="table table-bordered text-center">
                                    ...
                                    @foreach($taskData as $task)
                                        <tr class="col-12">
                                            <td style="font-family: 'Play', sans-serif; font-size: medium"
                                                class="col-11"> {{$task->memo_text}}</td>
                                            <td class="col-2">
                                                {!! Form::open([
        'method' => 'DELETE',
                        ...
                        <nav aria-label="Page navigation ">
                            <ul class="pagination justify-content-center">
                                ...
                                <li class="page-item {{ $taskData->previousPageUrl()==null ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $taskData->previousPageUrl() ?? '#' }}"> <span
                                ...
                                @for ($i=-$pagination_range; $i<=$pagination_range; $i++)
                                    ...
                                    @if ($taskData->currentPage()+$i > 0 && $taskData->currentPage()+$i <= $taskData->lastPage())
                                        <li class="page-item {{ $i==0 ? 'active' : '' }}">
                                            <a class="page-link"
                                               href="{{ $taskData->url($taskData->currentPage()+$i) }}">{{ $taskData->currentPage()+$i }}</a>
                                        </li>
                                    @endif
                                ...
                                @if ($taskData->currentPage() < $taskData->lastPage()-$pagination_range )
                                    ...
                                    <li class="page-item">
                                        <a class="page-link"
                                           href="{{ $taskData->url($taskData->lastPage()) ?? '#' }}">{{ $taskData->lastPage() }}</a>
                                    </li>

                                @endif

                                <li class="page-item {{ $taskData->nextPageUrl()==null ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $taskData->nextPageUrl() ?? '#' }}"><span
